...
VERSION="1.6.8";PORT=8099;HA=Path('/homeassistant');SHARE=Path('/share');CC=HA/'custom_components';CUSTOM=CC/'ecovacs';CLIENT_BACKUP_ROOT=HA/'ecovacs_doctor_client_backups';PROFILE_PATH=Path('/app/cqyi87_profile.py');OBS_FILE=SHARE/'deebot-y1pro-observations.jsonl';ACTIVE_FILE=SHARE/'deebot-y1pro-active-observation.json'
MATCH=re.compile(r'ecovacs|deebot|beepbop|cqyi87|30000|mqtt|capabilities|clean_V2|setSpeed|BatteryEvent|StateEvent|PositionsEvent|MapTraceEvent|FanSpeedEvent|Error while setting up ecovacs',re.I);TELEMETRY_MATCH=re.compile(r'30000|Received PUBLISH|Got message: topic=|Unknown message|BatteryEvent|StateEvent|PositionsEvent|MapTraceEvent|FanSpeedEvent|AvailabilityEvent|clean_V2|setSpeed|getBattery|getPos|getMapTrace|getChargeState|getWorkState|40001|40009|40011|40013|10000|10001',re.I)
...
def observation_finish(result,notes=''):
 ...
 r=json.loads(ACTIVE_FILE.read_text());r.update(finished_at=now_iso(),physical_result=str(result)[:100],notes=str(notes)[:500]);_,_,raw=get_logs('3m');r['diagnostic_log_lines']=[redact(x) for x in raw if TELEMETRY_MATCH.search(x)][-300:]
