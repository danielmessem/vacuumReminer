...
VERSION="1.7.1";PORT=8099;HA=Path('/homeassistant');SHARE=Path('/share');CC=HA/'custom_components';CUSTOM=CC/'ecovacs';CLIENT_BACKUP_ROOT=HA/'ecovacs_doctor_client_backups';PROFILE_PATH=Path('/app/cqyi87_profile.py');OBS_FILE=SHARE/'deebot-y1pro-observations.jsonl';ACTIVE_FILE=SHARE/'deebot-y1pro-active-observation.json'
...
def capture_telemetry():
 cid,name,raw=get_logs('20m');selected=[redact(x) for x in raw if TELEMETRY_MATCH.search(x)][-5000:];state_lines=[redact(x) for x in raw if STATE_MATCH.search(x)][-1000:];stamp=datetime.now().strftime('%Y%m%d-%H%M%S');filename=f'deebot-y1pro-telemetry-{stamp}.zip';out=SHARE/filename;report={'version':VERSION,'generated':now_iso(),'window':'20m','ha_vacuum_snapshot':ha_vacuum_snapshot(),'observations':load_observations(),'state_battery_timeline':state_lines,'payload_lines':selected}
...
class H(BaseHTTPRequestHandler):
 ...
 def do_GET(self):
  if '/download/' in self.path:
   filename=self.path.split('/download/',1)[1].split('?',1)[0]
   if not re.fullmatch(r'deebot-y1pro-telemetry-[0-9-]+\.zip',filename):return self.send_error(404)
   p=SHARE/filename
   if not p.is_file():return self.send_error(404)
   b=p.read_bytes();self.send_response(200);self.send_header('Content-Type','application/zip');self.send_header('Content-Disposition',f'attachment; filename="{filename}"');self.send_header('Content-Length',str(len(b)));self.end_headers();self.wfile.write(b);return
