...
def selected_entity(requested=None):
    ...
    if requested and requested in ids:ENTITY_FILE.write_text(requested); return requested
    if ENTITY_FILE.exists():
        s=ENTITY_FILE.read_text().strip()
        if s in ids:return s
    for c in candidates:
        txt=(c['entity_id']+' '+str(c['name'])).lower()
        if any(k in txt for k in ('beepbop','ecovacs','deebot')):ENTITY_FILE.write_text(c['entity_id']); return c['entity_id']
    if len(candidates)==1:ENTITY_FILE.write_text(candidates[0]['entity_id']); return candidates[0]['entity_id']
    return None
...
def observation_start(action,execute=False,entity_id=None):
    ...
    if not entity:return {'ok':False,'message':'Select the vacuum entity first','candidates':vacuum_candidates()}
...
HTML="""<!doctype html><meta name=viewport content='width=device-width,initial-scale=1'><title>DEEBOT Y1 PRO Tools</title>
<style>
*{box-sizing:border-box}body{font-family:system-ui;max-width:1000px;margin:24px auto;padding:0 18px;background:#111827;color:#e5e7eb}button,select,input{padding:12px 14px;border:0;border-radius:8px;font-weight:650;font-size:16px}.p{background:#2563eb;color:white}.g{background:#16a34a;color:white}.card{border:1px solid #374151;border-radius:10px;padding:16px;margin:14px 0;background:#1f2937}.field{display:block;width:100%;margin:14px 0}.field label{display:block;margin-bottom:7px;font-weight:700}.field select{display:block;width:100%;max-width:100%;background:#fff;color:#111827}.row{display:flex;gap:10px;align-items:stretch;flex-wrap:wrap;margin:14px 0}.row select{flex:1 1 280px;min-width:0;background:#fff;color:#111827}.row button{flex:0 0 auto}#refresh{margin-top:8px;background:#374151;color:#fff}#answer{margin-top:18px;padding-top:14px;border-top:1px solid #4b5563}#answer .row input{flex:1 1 280px;min-width:0}pre{background:#030712;padding:14px;white-space:pre-wrap;max-height:600px;overflow:auto;border-radius:8px}.hint{color:#9ca3af}@media(max-width:600px){body{margin:12px auto;padding:0 10px}.card{padding:12px}.row{display:block}.row select,.row input,.row button{display:block;width:100%;margin:8px 0}.field select,#refresh{width:100%}}
</style>
<h1>DEEBOT Y1 PRO Diagnostics</h1><p>Version <b>1.6.3</b></p>
<div class=card><h2>One-click physical command test</h2><p class=hint>Select the Y1 PRO once, then choose an action. The add-on sends it and only asks what physically happened.</p>
<div class=field><label for=entity>Vacuum entity</label><select id=entity></select><button id=refresh onclick='loadEntities()'>Refresh entities</button></div>
<div class=row><select id=attempt><option>Start cleaning</option><option>Pause</option><option>Resume</option><option>Return to dock</option><option>Stop cleaning</option></select><button class=p onclick='runTest()'>Run command & capture</button></div>
<div id=answer style='display:none'><p><b>What did the robot physically do?</b></p><div class=row><select id=result><option>Started cleaning</option><option>Paused</option><option>Resumed cleaning</option><option>Returned to dock</option><option>Stopped cleaning</option><option>No physical response</option><option>Other / unexpected</option></select><input id=notes placeholder='Optional note'><button class=g onclick='finishObs()'>Save result</button></div></div><p id=obs>Loading vacuum entities...</p></div>
<div class=row><button onclick="go('./api/diagnose')">Run diagnosis</button><button onclick="go('./api/telemetry')">Export telemetry</button></div><pre id=o>Ready.</pre>
