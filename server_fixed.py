...
VERSION = "0.9.7"
HTML = f'''<!doctype html><html><head><meta name="viewport" content="width=device-width"><title>DEEBOT Diagnostics</title><style>body{{font-family:system-ui;max-width:900px;margin:30px auto;padding:0 18px}}button{{font-size:16px;padding:12px 18px;border:1px solid #aaa;border-radius:8px;background:#fff;cursor:pointer}}.card{{padding:16px;border:1px solid #ddd;border-radius:10px;margin:14px 0}}</style></head><body><h1>DEEBOT Y1 PRO Diagnostics</h1><div class="card">Version <b>{VERSION}</b></div><div class="card"><h2>Deep Y1 PRO capture</h2><p>The button submits directly to the application root. The ZIP is returned directly by the server.</p><form action="/" method="post"><button type="submit">Run Deep Capture + Download ZIP</button></form></div><div class="card"><a href="/api/diagnostic">Run normal diagnostic</a></div><div class="card"><a href="/api/core-inspection-script">Show Core inspection script</a></div></body></html>'''
