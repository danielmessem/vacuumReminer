...
`10000` is a partial-update stream. A handler must not require all fields to be present and must not reset unrelated state when a field is omitted.

Physical-validation status should be stated conservatively. Start smart cleaning is physically verified. Other captured commands should not be described as physically verified without an explicit physical test record.
