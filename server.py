...
LOG_RE=re.compile(r'ecovacs|deebot|beepbop|LPATPGFR|cqyi87|please update|unsupported|exception|traceback|auth|discover|error',re.I)
...
def config_entries():
    try:
        es=json.loads(read(HA_CONFIG/'.storage/core.config_entries',5000000)).get('data',{}).get('entries',[])
...
def core_logs(lines=3000):
    # /core/logs is the documented Supervisor endpoint; /core/logs/latest can be restricted by role/version.
    r=api(f'/core/logs?lines={lines}&no_colors',headers={'Accept':'text/plain'})
    d=data(r); text=d if isinstance(d,str) else json.dumps(d,default=str)
    relevant=[x for x in text.splitlines() if LOG_RE.search(x)]
    return {'endpoint':'/core/logs','lines_requested':lines,'total_lines':len(text.splitlines()),'match_count':len(relevant),'matching_lines':relevant[-3000:],'request':{k:v for k,v in r.items() if k!='data'}}
...
def error_log():
    ...
    return {'content':d if isinstance(d,str) else json.dumps(d,default=str),'request':{k:v for k,v in r.items() if k!='data'}}

def services():
    d=data(api('/core/api/services'))
    return [x for x in d if x.get('domain') in ('vacuum','ecovacs','deebot','homeassistant','logger')] if isinstance(d,list) else d
...
def base_snapshot():
    return {'timestamp':now(),'config_entries':config_entries(),'devices':registry('core.device_registry','devices'),'entities':registry('core.entity_registry','entities'),'states':states(),'logs':core_logs(3000),'error_log':error_log()}
...
def reload_ecovacs():
    ...
    eid=targets[0].get('entry_id'); before=base_snapshot()
    call=api('/core/api/services/homeassistant/reload_config_entry','POST',{'entry_id':eid})
    time.sleep(8); after=base_snapshot()
...
def deep_capture():
    ...
    debug_result=set_debug(True)
    reload_result=reload_ecovacs()
    time.sleep(4)
    debug_logs=core_logs(3000)
    debug_error=error_log()
    restore_result=set_debug(False)
    after=base_snapshot()
    return {'started_at':before['timestamp'],'debug_enabled':safe('',debug_result),'reload':reload_result,'debug_capture':{'logs':debug_logs,'error_log':debug_error},'debug_restored':safe('',restore_result),'final_state':after}

def diagnostic(extra=None):
    result={'generated_at':now(),'add_on':{'version':VERSION},'environment':{'python':subprocess.run(['python3','--version'],capture_output=True,text=True).stdout.strip(),'arch':os.uname().machine,'hostname':socket.gethostname()},'home_assistant':safe('',api('/supervisor/info')),'core_config':safe('',api('/core/api/config')),'deebot_entities':states(),'config_entries':config_entries(),'registry':{'devices':registry('core.device_registry','devices'),'entities':registry('core.entity_registry','entities')},'integration':integration_info(),'services':services(),'network':network(),'core_logs':core_logs(3000),'supervisor_logs':supervisor_logs(500),'error_log':error_log(),'known_y1_pro_methods':Y1_METHODS}
