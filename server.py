...
def set_debug(enabled):
    ...
    return supervisor("/core/api/services/logger/set_level", "POST", {
        "homeassistant.components.ecovacs": level,
        "ecovacs": level,
        "deebot_client": level,
    })
...
def reload_ecovacs():
    ...
    if not entries:
        return {"ok": False, "error": "No Ecovacs config entry found"}
...
def snapshot():
    ...
    return {
        "timestamp": now(),
        "config_entries": redact(config_entries()),
        "states": states(),
        "logs": logs,
        "error_log": error_log(),
        "client_inspection": inspect_client("\n".join(logs["matching_lines"])),
    }
...
def deep_capture(job):
    try:
        ...
        script.write_text(core_inspection_script())
        script.chmod(0o755)
        ...
        captured = core_logs(10000)
        errors = error_log()

        progress(job, 72, "Analysing cqyi87 / clean_V2 / work-mode evidence")
        text = "\n".join(captured["matching_lines"])
        evidence = {
            "cqyi87_lines": [x for x in captured["matching_lines"] if "cqyi87" in x],
            "clean_v2_lines": [x for x in captured["matching_lines"] if "clean_V2" in x],
            "work_mode_lines": [x for x in captured["matching_lines"] if re.search(r"workMode|setWorkMode|getWorkMode|workState|motionState", x, re.I)],
            "p2p_lines": [x for x in captured["matching_lines"] if "p2p" in x.lower()],
            "command_count": len(re.findall(r"cmdName", text)),
        }
        ...
        result = {
            "started_at": JOBS[job]["started_at"],
            "finished_at": now(),
            "add_on_version": VERSION,
            "before": before,
            "reload": reload_result,
            "capture": {"logs": captured, "error_log": errors, "evidence": evidence},
            "after": after,
        }
        ...
        with LOCK:
            JOBS[job].update(status="complete", percent=100, message="Complete", file=str(output))
    except Exception as exc:
        try:
            set_debug(False)
        except Exception:
            pass
        with LOCK:
            JOBS[job].update(status="error", percent=100, message=str(exc), error=str(exc))
...
def diagnostic():
    return {
        "generated_at": now(),
        "add_on_version": VERSION,
        "environment": {"python": subprocess.run(["python3", "--version"], capture_output=True, text=True).stdout.strip(), "arch": os.uname().machine, "hostname": socket.gethostname()},
        "config_entries": redact(config_entries()),
        "states": states(),
        "core_logs": core_logs(),
        "error_log": error_log(),
...
<style>
body{font-family:system-ui;max-width:1050px;margin:25px auto;padding:0 18px}
button{padding:11px 15px;margin:4px;border:1px solid #bbb;border-radius:8px;background:#fff;cursor:pointer}
.card{border:1px solid #ddd;border-radius:10px;padding:14px;margin:12px 0}
#log{background:#111;color:#eee;padding:12px;border-radius:8px;min-height:180px;white-space:pre-wrap;font:12px monospace;max-height:500px;overflow:auto}
.bar{height:18px;background:#ddd;border-radius:10px;overflow:hidden}.fill{height:100%;width:0%;background:#1976d2}
.status{font-weight:600} a{text-decoration:none}
</style></head><body>
<h1>DEEBOT Y1 PRO Diagnostics</h1><p>Version <b>VERSION_PLACEHOLDER</b></p>
<div class="card"><h2>Deep Y1 PRO Capture</h2>
<p>Captures the Y1 PRO / cqyi87 traffic and shows live progress. It does not patch the Ecovacs integration.</p>
<button id="run" onclick="runCapture()">Run Deep Capture</button>
<div class="bar"><div id="fill" class="fill"></div></div>
<p class="status" id="status">Ready</p><div id="log">Press Run Deep Capture to start.</div><p id="download"></p></div>
<div class="card"><a href="api/diagnostic">Run Normal Diagnostic</a> &nbsp; | &nbsp; <a href="api/core-inspection-script">Show Core Inspection Script</a></div>
<script>
let timer;
async function runCapture(){
 const button=document.getElementById('run'); button.disabled=true;
 document.getElementById('download').innerHTML=''; document.getElementById('log').textContent='Starting...';
 const response=await fetch('api/deep',{method:'POST'}); const job=await response.json();
 if(!job.job){document.getElementById('status').textContent='ERROR: '+JSON.stringify(job);button.disabled=false;return;}
 timer=setInterval(async()=>{
   const s=await fetch('api/job/'+job.job).then(r=>r.json());
   document.getElementById('fill').style.width=s.percent+'%';
   document.getElementById('status').textContent=s.percent+'% — '+s.message;
   document.getElementById('log').textContent=(s.events||[]).map(e=>'['+e.time+'] '+e.message).join('\n');
   document.getElementById('log').scrollTop=999999;
   if(s.status==='complete'||s.status==='error'){
     clearInterval(timer); button.disabled=false;
     if(s.file) document.getElementById('download').innerHTML='<a href="api/download/'+job.job+'">Download diagnostic ZIP</a>';
   }
 },500);
}
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if match:
            ...
            p = Path(job["file"])
            return self.send_bytes(200, p.read_bytes(), "application/zip", f'attachment; filename="{p.name}"')
