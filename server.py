...
PORT=8099; VERSION='0.4.0'; HA_CONFIG=Path('/homeassistant'); SHARE=Path('/share'); SUPERVISOR='http://supervisor'
...
def config_entries():
    ...
    try:
        es=json.loads(read(p,5000000)).get('data',{}).get('entries',[]); selected=[e for e in es if str(e.get('domain','')).lower() in ('ecovacs','deebot')]
...
def logs(lines=3000):
    r=api(f'/core/logs/latest?lines={lines}&no_colors'); d=data(r); text=d if isinstance(d,str) else json.dumps(d,default=str)
...
def services():
    ...
    return [x for x in d if x.get('domain') in ('vacuum','ecovacs','deebot','homeassistant')] if isinstance(d,list) else d
...
def base_snapshot():
    return {'timestamp':now(),'config_entries':config_entries(),'devices':registry('core.device_registry','devices'),'entities':registry('core.entity_registry','entities'),'states':states(),'logs':logs(3000),'error_log':error_log()}
...
def reload_ecovacs():
    ...
    # Use Home Assistant's reload_config_entry service rather than touching credentials or storage.
    call=api('/core/api/services/homeassistant/reload_config_entry','POST',{'entry_id':eid})
    time.sleep(8); after=base_snapshot()
    before_devices=before['devices']; after_devices=after['devices']; before_entities=before['entities']; after_entities=after['entities']
    return {'entry_id':eid,'service_result':safe('',call),'before':before,'after':after,'discovery_diff':{'devices_before':len(before_devices),'devices_after':len(after_devices),'entities_before':len(before_entities),'entities_after':len(after_entities),'new_devices':[x for x in after_devices if x not in before_devices],'new_entities':[x for x in after_entities if x not in before_entities]}}
