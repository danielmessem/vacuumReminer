...
from installed_client_inspector import inspect as inspect_client

PORT=8099; VERSION='0.7.0'; HA_CONFIG=Path('/homeassistant'); SHARE=Path('/share'); SUPERVISOR='http://supervisor'
SENSITIVE=re.compile(r'(token|password|secret|username|access_token|refresh_token|authorization|cookie)',re.I)
LOG_RE=re.compile(r'ecovacs|deebot|beepbop|LPATPGFR|cqyi87|please update|unsupported|exception|traceback|auth|discover|error|deebot[-_ ]?client',re.I)
...
def snapshot():
    logs=core_logs(5000)
    return {'timestamp':now(),'config_entries':config_entries(),'devices':registry('core.device_registry','devices'),'entities':registry('core.entity_registry','entities'),'states':states(),'logs':logs,'error_log':error_log(),'client_inspection':inspect_client(logs.get('matching_lines_text','\n'.join(logs.get('matching_lines',[]))))}
